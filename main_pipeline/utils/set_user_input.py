...
def set_arguments_values():
    ...
    arg_input_geometry.set_argument_help_message(
        "Path to the input geometry (.geojson) to intersect sentinel data (relative to the parent folder)"
    )
    arg_input_geometry.set_argument_default_value(
        "./main_pipeline/input_geometries/doberitz_multipolygon.geojson"
    )
    ...
    arg_folder.set_argument_help_message(
        "Folder path to write the resulting data (relative to the parent folder)"
    )
    ...
    arg_start_date.set_argument_help_message(
        "Start date to process format DD-MM-YYYY (hyphen sep)"
    )
    default_end_date = datetime.date.today()
    default_start_date = (default_end_date - datetime.timedelta(days=30)).strftime(
        "%d-%m-%Y"
    )
    ...
    arg_end_date.set_argument_help_message(
        "End date to process format DD-MM-YYYY (hyphen sep)"
    )
        ...
        if arg.argument_type_name == "Date":
            try:
                if type(input_parameters_raw[arg.argument_name]) is datetime.datetime:
                    input_parameters_checked[arg.argument_name] = input_parameters_raw[
                        arg.argument_name
                    ]
                elif type(input_parameters_raw[arg.argument_name]) is str:
                    temp_argument_type_date = datetime.datetime.strptime(
                        input_parameters_raw[arg.argument_name], "%d-%m-%Y"
                    )
                    input_parameters_checked[
                        arg.argument_name
                    ] = temp_argument_type_date
            except:
                print(
                    "wrong date format {0} date should match dd-mm-yyyy. exiting with error".format(
                        arg.argument_name
                    )
                )
                exit()
        elif (
            arg.argument_type_name == "Integer"
            or arg.argument_type_name == "Integer_no_zero"
        ):
            if not type(input_parameters_raw[arg.argument_name]) is int:
                raise TypeError("Only integers are allowed")
                exit()
        elif (
            arg.argument_type_name == "Integer_no_zero"
            and input_parameters_raw[arg.argument_name] < 0
        ):
...
def set_arguments_pipeline():
    ...
    days_timedelta = (
        input_parameters_checked["end_date"] - input_parameters_checked["start_date"]
    )
